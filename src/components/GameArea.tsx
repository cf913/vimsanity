...
const GameArea: React.FC<GameAreaProps> = ({ level, isMuted }) => {
  ...
  const [wordPosition, setWordPosition] = useState<WordPosition>({ wordIndex: 0, charIndex: 0 })
  const [wordTarget, setWordTarget] = useState<WordPosition>({ wordIndex: 2, charIndex: 0 })
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (level === 1) {
      ...
      switch (e.key) {
        case 'w': // Move to start of next word
          if (wordPosition.wordIndex < words.length - 1) {
            newWordPos = { wordIndex: wordPosition.wordIndex + 1, charIndex: 0 }
          }
          break
        ...
        case 'l': // Move one character right (additional movement for fine control)
          if (wordPosition.charIndex < words[wordPosition.wordIndex].length - 1) {
            newWordPos = { 
              wordIndex: wordPosition.wordIndex, 
      ...
      if (newWordPos.wordIndex === wordTarget.wordIndex && newWordPos.charIndex === wordTarget.charIndex) {
        if (!isMuted) {
          successSound.currentTime = 0
          successSound.play()
        }
    ...
    <div className="flex flex-col items-center gap-8">
            ...
            {words.map((word, wordIdx) => (
              <span key={wordIdx} className="relative mr-2 inline-block">
                {word.split('').map((char, charIdx) => {
                  const isPlayerPosition = wordIdx === wordPosition.wordIndex && charIdx === wordPosition.charIndex;
                  const isTargetPosition = wordIdx === wordTarget.wordIndex && charIdx === wordTarget.charIndex;
      ...
      <div className="flex gap-4 text-zinc-400">
        {level === 1 ? (
          ...
          <>
            <kbd className="px-3 py-1 bg-zinc-800 rounded-lg">w</kbd>
            <kbd className="px-3 py-1 bg-zinc-800 rounded-lg">b</kbd>
            <kbd className="px-3 py-1 bg-zinc-800 rounded-lg">e</kbd>
            <kbd className="px-3 py-1 bg-zinc-800 rounded-lg">h</kbd>
            <kbd className="px-3 py-1 bg-zinc-800 rounded-lg">l</kbd>
          </>
        )}

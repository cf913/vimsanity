...
const GridMovementLevel: React.FC<GridMovementLevelProps> = ({ isMuted }) => {
  ...
  const [targetEaten, setTargetEaten] = useState<{ x: number; y: number } | null>(null)
  ...
  useEffect(() => {
    if (targetEaten) {
      const timer = setTimeout(() => setTargetEaten(null), 300)
      return () => clearTimeout(timer)
    }
  }, [targetEaten])
  ...
  const { lastKeyPressed } = useKeyboardHandler({
    ...
    dependencies: [position, target, level.score],
              ...
              <div className="relative flex justify-center">
                ...
                >
                  {Array.from({ length: gridSize * gridSize }).map((_, index) => {
                    ...
                    const isTarget2 = x === target.x && y === target.y
                    const isTargetEaten =
